...
const fragmentShader = `
  ...
  void main() {
    ...
    vec3 base = cppn.rgb;
    float t = clamp((base.r + base.g + base.b) / 3.0, 0.0, 1.0);
    // Very subtle palette
    vec3 colA = vec3(0.15, 0.25, 0.45);  // dark blue
    vec3 colB = vec3(0.35, 0.15, 0.35);  // dark purple
    vec3 palette = mix(colA, colB, t);
    vec3 color = palette * 0.3; // very dim
    color = clamp(color, 0.0, 1.0);
    // Soft vignette fade
    float r = length(uv);
    float fade = 1.0 - smoothstep(0.3, 1.3, r);
    float alpha = 0.12 * fade; // barely visible subtle animation
...
function ShaderBackground({ reducedMotion }: { reducedMotion: boolean }) {
  ...
  if (reducedMotion) {
    return (
      <div className="bg-black absolute inset-0 -z-10 w-full h-full" aria-hidden="true">
  ...
  return (
    <div ref={canvasRef} className="bg-black absolute inset-0 z-0 w-full h-full" aria-hidden="true">
      <Canvas
        camera={camera}
...
// Free alternative to GSAP SplitText - splits title into words
function AnimatedTitle({ text, reducedMotion }: { text: string; reducedMotion: boolean }) {
  ...
  if (reducedMotion) {
    return (
      <h1 className="text-left text-5xl md:text-6xl lg:text-7xl font-bold mb-6 tracking-tight text-foreground">
        {text}
      </h1>
    );
  }
  ...
  return (
    <h1 
      ref={titleRef}
      className="text-left text-5xl md:text-6xl lg:text-7xl font-bold mb-6 tracking-tight text-foreground"
    >
      {words.map((word, index) => (
        <span key={`w-${index}`} className="word inline-block mr-2">
          {word}
        </span>
      ))}
    </h1>
...
export default function NeuralNetworkHero({
  title,
  description,
  badgeText = "AI/ML Engineer",
  badgeLabel = "Available",
  ctaButtons = [
    { text: "View Projects", href: "#projects", primary: true },
    { text: "Contact Me", href: "#contact" }
  ],
  microDetails = ["Python & PyTorch", "Production RAG", "Agent Orchestration"]
}: HeroProps) {
  ...
  const [reducedMotion, setReducedMotion] = useState(() => {
    ...
    return Boolean(media || manual);
  });

  useEffect(() => {
    if (typeof window === 'undefined') return;
    const onToggle = (e: any) => setReducedMotion(!!e?.detail?.enabled);
    window.addEventListener('reduced-motion-changed', onToggle as any);
    return () => window.removeEventListener('reduced-motion-changed', onToggle as any);
  }, []);
  ...
  const runEntranceAnimation = () => {
    ...
    if (paraRef.current) {
      tl.to(paraRef.current, { autoAlpha: 1, y: 0, duration: 0.5 }, 1.3);
    }
    if (ctaRef.current) {
      tl.to(ctaRef.current, { autoAlpha: 1, y: 0, duration: 0.5 }, 1.5);
    }
    if (microRef.current) {
      const microItems = microRef.current.querySelectorAll('li');
      tl.to(microItems, { autoAlpha: 1, y: 0, duration: 0.5, stagger: 0.1 }, 1.7);
    }
  };
  ...
  useEffect(() => {
    ...
    if (typeof document === 'undefined' || !(document as any).fonts || !document.fonts?.ready) {
      runEntranceAnimation();
      return;
    }

    // Wait until fonts are loaded so text animation measures correctly
    (document as any).fonts.ready
      .then(runEntranceAnimation)
      .catch(runEntranceAnimation);
  }, [reducedMotion]);

  return (
    <section ref={sectionRef} id="hero" className="relative h-screen w-screen overflow-hidden scroll-mt-28">
      ...
      <div className="relative z-10 mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16">
        <div ref={badgeRef} className="inline-flex items-center gap-3">
          <Badge variant="outline" className="uppercase tracking-[0.08em] text-[10px] font-medium">
            {badgeLabel}
          </Badge>
          <span className="h-1 w-1 rounded-full bg-white/30" aria-hidden="true" />
          <span className="text-xs font-medium tracking-tight text-muted-foreground">{badgeText}</span>
        </div>

        <AnimatedTitle text={title} reducedMotion={reducedMotion} />

        <p ref={paraRef} className="section-description mx-0 text-left max-w-2xl">
          {description}
        </p>
        ...
        <div ref={ctaRef} className="flex flex-wrap items-center gap-3 pt-2">
          {ctaButtons.map((button, index) => (
            <Button
              ...
              size="lg"
              className={(button.primary ? 'gap-2' : 'gap-2 backdrop-blur-sm') + ' rounded-xl px-5'}
            >
              <a href={button.href}>
        ...
        <ul ref={microRef} className="mt-6 flex flex-wrap gap-2">
          {microDetails.map((detail, index) => (
            <li key={index} className="text-[11px] text-muted-foreground px-2.5 py-1 rounded-full bg-white/5 border border-white/10">
              {detail}
            </li>
          ))}
        </ul>
